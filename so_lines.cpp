...
void SoLines::build ( const GsArray<GsVec>& lines, const GsColor& c )
 {
	//_lines.build ( _superq.NL, GsColor::yellow ); from app_window.cpp
	/* 
	Summary: Normal vecors are created from the object class (so capsule.cpp), which uses GsArray to save vectors.
	Then, another class (so_lines, for this case) simply copy that array and buffer it to display. 
